...
const EditListBookForSelling = ({sellBooksDetails , UserBookDetails}) => {
  ...
  const [bookname, setBookname] = useState(sellBooksDetails.name);
  const [authorname, setAuthorname] = useState(sellBooksDetails.author);
  const [description, setDescription] = useState(sellBooksDetails.description);
  const [price, setPrice] = useState(sellBooksDetails.price);
  const [media, setMedia] = useState("");
  const [img, setImg]  = useState("")
  const [condition, setCondition] = useState(sellBooksDetails.condition);
  const [category, setCategory] = useState(sellBooksDetails.category);
  const [adress, setAdress] = useState(sellBooksDetails.adress);
  const [landmark, setLandmark] = useState(sellBooksDetails.landmark);
  const [country, setCountry] = useState(sellBooksDetails.country);
  ...
  const [study, setStudy] = useState(sellBooksDetails.study);
  const [college, setCollege] = useState(sellBooksDetails.college);
  const [otherCollege, setOtherCollege] = useState(sellBooksDetails.college);
  const [school, setSchool] = useState(sellBooksDetails.school);
  ...
  const handleSubmit = async (e) => {
    ...
    const mediaUrl = await imgUpload();
     ...
     if(college === "other"){
       ...
       if (bookData.error) {
         ...
         return toast({
           ...
           description: "We've Updated your book for selling.",
           status: "success",
           duration: 5000,
           isClosable: true,
         });
  ...
  const handleReset = () => {
    setBookname(sellBooksDetails.name);
    setAuthorname(sellBooksDetails.author);
    setDescription(sellBooksDetails.description);
    setPrice(sellBooksDetails.price);
    setCondition(sellBooksDetails.condition);
    setCategory(sellBooksDetails.category);
    setAdress(sellBooksDetails.adress);
    setLandmark(sellBooksDetails.landmark);
    setCountry(sellBooksDetails.country);
    setCity(sellBooksDetails.city);
    setState(sellBooksDetails.state);
...
export default EditListBookForSelling;
